...
router.get('/2', async (req, res, next) => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const x = data.map(entry => parseInt(entry._id.연도));
        const y = data.map(entry => parseInt(entry._id.월));
        const z = data.map(entry => entry.계약수);
        ...
        const trace = {
            ...
            hovertemplate: '%{x}년 %{y}월: %{z}회<extra></extra>'
        };
